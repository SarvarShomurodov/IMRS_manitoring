...
@endif
<form action="{{ route('doctorate.update',$students->id) }}" method="POST">
    @csrf
    @method('PUT')
    <div class="row">
        <div class="form-group col-lg-4">
            ...
            <select name="dokt_id" class="form-control" id="dokt_id" class="form-control" required>
                ...
                @foreach($studentsid as $whogiven)
                    ...
                    <option value="{{ $whogiven->id }}" {{ old('dokt_id', $students->dokt_id) == $whogiven->id ? 'selected' : '' }}>
                        {{ $whogiven->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group col-lg-2">
            ...
            <input type="soni" name="soni" id="soni" class="form-control" value="{{ old('soni', $students->soni) }}" min="0" placeholder="қабул сони" required>
        </div>

        <div class="form-group col-lg-3">
            <label for="soni"><b>Макроиқтисодиёт 08.00.02 сони</b></label>
            <input type="makro" name="makro" id="makro" class="form-control" value="{{ old('makro',$students->makro) }}" min="0" placeholder="Макроиқтисодиёт 08.00.02 сони">
        </div>

        <div class="form-group col-lg-3">
            <label for="soni"><b>Минтақавий иқтисодиёт 08.00.12 сони</b></label>
            <input type="minta" name="minta" id="minta" class="form-control" value="{{ old('minta',$students->minta) }}" min="0" placeholder="Минтақавий иқтисодиёт 08.00.12 сони">
        </div>
        ...
        <div class="form-group col-lg-2">
          ...
          <select name="quarters_id" class="form-control" id="quarters_id" class="form-control" required>
              ...
              @foreach($quarters as $quarter)
                    <option value="{{ $quarter->id }}" {{ old('quarters_id', $students->quarters_id) == $quarter->id ? 'selected' : '' }}>
                        {{ $quarter->name }}
                    </option>
              @endforeach
          </select>
